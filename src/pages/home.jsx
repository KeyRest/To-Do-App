...
export default function Home() {
    ...
    const endOfWeek = today.add(7, 'day').endOf('day');

    // Filtrado de tareas
    const overdueDuties = sortedDuties.filter(duty => dayjs(duty.date).isBefore(today));
    const todayDuties = sortedDuties.filter(duty => dayjs(duty.date).isSame(today, 'day'));
    const nextWeekDuties = sortedDuties.filter(duty => dayjs(duty.date).isAfter(endOfToday) && dayjs(duty.date).isBefore(endOfWeek));

    const filteredDuties = sortedDuties.filter(duty =>
                ...
                <div className='hidden xl:flex justify-between px-12 col-span-4 items-center h-full'>
                    ...
                    <div className='flex items-center justify-center w-full gap-8 h-full '>
                        <div className='flex items-center justify-end w-full h-full '>
                            <Link to={"/"}>
                                <svg className='hover:scale-125 duration-300' xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="none" viewBox="0 0 24 24"><path stroke="#000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 4h3c1.1046 0 2 .8954 2 2v12c0 1.1046-.8954 2-2 2h-3M8 8l-4 4m0 0 4 4m-4-4h12" /></svg>
                            </Link>
                ...
                <div className='col-span-4 grid grid-cols-4 2xl:gap-20 h-full w-full'>
                    ...
                    <div className='col-span-4 xl:col-span-2 flex flex-col gap-8 m-auto w-full lg:w-[80%] xl:w-full xl:h-full xl:scale-90 2xl:scale-100'>
                        <div className='flex justify-between items-center md:p-8 xl:p-0'>
                            <Link to={"/"}>
                                <svg className='hover:scale-125 duration-300 xl:hidden' xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="none" viewBox="0 0 24 24"><path stroke="#000" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 4h3c1.1046 0 2 .8954 2 2v12c0 1.1046-.8954 2-2 2h-3M8 8l-4 4m0 0 4 4m-4-4h12" /></svg>
                            </Link>
                        ...
                        <div className='flex flex-col items-center gap-2 overflow-y-scroll h-[60vh] lg:h-[65vh] xl:h-[31.5rem] 2xl:h-[31.5rem] 3xl:h-[36.5rem] w-full m-auto'>
                            {!showAll ? (
                                <>
                                    <h2 className='text-4xl font-bold p-5'>Overdue</h2>
                                    ...
                                    <h2 className='text-4xl font-bold p-5'>Today</h2>
                                    ...
                                    <h2 className='text-4xl font-bold p-5'>Next 7 Days</h2>
                                    {nextWeekDuties.map((duty) => (
                                        <Task
                                            key={duty.id}
                                            title={duty.name}
                                            date={duty.date}
                                ...
                                <>
                                <h2 className='text-4xl font-bold p-5'>All duties</h2>
                        ...
                        <button onClick={() => setShowAll(!showAll)} className="mt-4 p-2 bg-blue-500 text-white rounded">
                            {showAll ? 'Show Filtered Duties' : 'Show All Duties'}
                        </button>
                    </div>
